...
**Note**: `summary` is auto-generated by AI when `ANTHROPIC_API_KEY` is configured on the server. It appears asynchronously after the notification is created (delivered via SSE "updated" event). Messages shorter than 80 characters are not summarized.
